...
Chat with xAI's Grok models from Clawdbot. Supports text chat, vision, X/Twitter search, web search, and all Grok models including Grok-4.
...
node scripts/search-x.js --allowed-handles @remotion_dev "new features"
...
node scripts/search-x.js --handles @remotion_dev --from-date 2025-01-01 --image-understanding "release notes"
```
...
node scripts/web-search.js --exclude-domains reddit.com --max-results 3 "pricing comparison"
...
- Chat with Grok models (text + vision)
- Search X/Twitter with advanced filters (handles, date ranges, media understanding)
- Web search with domain filters and citations

## Notes

- xai now replaces the legacy xai-search skill for X/Twitter and web search capabilities.
