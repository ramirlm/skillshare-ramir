...
- Posting tweets (cookie-based approaches often get blocked by bot detection)
- Official API access is needed for reliability

Para **leitura** (timeline, search, mentions), prefira uma abordagem de busca (ex.: skill `grok-search`) ou cole aqui o texto/print do tweet para eu resumir/analisar.
...
Como o `bird` foi removido, para ler tweets/threads você pode:
- Usar uma skill de busca (ex.: `grok-search`) quando precisar encontrar conteúdo no X.
- Ou colar aqui o texto do tweet/thread, ou mandar print/vídeo, que eu resumo/análise.
